...
public class CertificateDaoImpl implements CertificateDao{
	...
	public void updateCertificate(Certificate certificate) {
		String sql = "UPDATE CERTIFICATE SET CERTIFICATE_NAME=?, ISSUE_DATE=?, END_DATE=?, FILE_PATH=?, VERIFICATION_STATUS=?, VERIFIED_BY_ID=?, VERIFIED_DATE=?, MODIFIED_BY=0, MODIFIED_TIME=SYSDATE()) WHERE CERTIFICATE_ID=?";
		...
		try
		{
			...
			System.out.println("issue date formated: "+new Date(format.parse(certificate.getIssueDate()).getTime()));
			System.out.println("end date formated: "+new Date(format.parse(certificate.getEndDate()).getTime()));
			System.out.println("verification date formated: "+new Date(format.parse(certificate.getVerificationDate()).getTime()));
	...
	public Certificate getCertificate(Long certificateId) {
		...
		return jdbcTemplate.query(sql, new Object[] {certificateId}, (ResultSetExtractor<Certificate>) rs -> {
				...
				certificate.setVerificationDate(dateFormat.format(rs.getDate("VERIFIED_DATE")));
